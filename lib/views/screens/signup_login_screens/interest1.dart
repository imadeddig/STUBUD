...
Future<void> updateUserInterests(String userId, List<String> interests) async {
  final url = Uri.parse('$flaskBaseUrl/update_interests');
  
  try {
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({'userID': userId, 'interests': interests}),
    );
 ...
 Future<void> _fetchInterests() async {
  try {
    final response = await http.get(Uri.parse('$flaskBaseUrl/fetch_interests'));
                      ...
                      children: _filteredLabel.map((label) {
                        ...
                        return GestureDetector(
                          onTap: () async {
                            ...
                            try {
                             await updateUserInterests(widget.userID, _selected5.toList());
